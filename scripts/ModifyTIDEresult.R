...
ModifyTIDEresult <- function(gene){
  ...
  write.table(TIDE.target, paste0("./output_TIDE/", gene, "/Modified.TIDE.result.first_last_15.with.",target.gene,".expr.txt" ), sep = ",", row.names = FALSE )
  ...
  write.table(TIDE.target, paste0("./output_TIDE/", gene, "/Modified.TIDE.result.first_last_25.with.",target.gene,".expr.txt" ), sep = ",", row.names = FALSE )
  ...
  first_last_30_TIDE.result = first_last_30_TIDE.result %>% 
    add_column(expr_high_or_low = ifelse(first_last_35_TIDE.result$gene.expr > 0, "high", "low"), .after = 1)
  write.table(TIDE.target, paste0("./output_TIDE/", gene, "/Modified.TIDE.result.first_last_30.with.",target.gene,".expr.txt" ), sep = ",", row.names = FALSE )
